...
public class Fecha {
    ...
    public static String getFecha(boolean fechaSoloNumeros, boolean fechaConSlash){
        construirFecha();
        if(fechaSoloNumeros){
            return Hora.cambiar(Integer.parseInt(dia)) + (fechaConSlash?"/":"-") + (fecha.get(Calendar.MONTH)+1) + (fechaConSlash?"/":"-") +anio;
        }else{
            return Hora.cambiar(Integer.parseInt(dia)) + (fechaConSlash?"/":"-") + mes + (fechaConSlash?"/":"-") +anio;
        }
    }
